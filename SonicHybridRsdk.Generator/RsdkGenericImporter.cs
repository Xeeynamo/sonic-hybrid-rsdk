...
if (context.Version == RsdkVersion.RSDKv3)
{
Create(dstFile, stream => new RSDKv3.StageConfig
{
Objects = src.Objects,
Sfx = src.Sfx,
...
else if (context.Version == RsdkVersion.RSDKv4)
{
Create(dstFile, stream => new RSDKv4.StageConfig
{
Objects = src.Objects,
Sfx = src.Sfx,
UnknownData = src.UnknownData,
...
{
throw new ArgumentException("Invalid RSDK version specified in context.");
}
}
        ...
        Action<Context, IStageAct> postAction = null)
    {
        ...
        if (context.Version == RsdkVersion.RSDKv3)
        {
            Create(dstFile, stream => new RSDKv3.StageAct
            {
                Title = act.Title,
                Layers = act.Layers,
                Width = act.Width,
                Height = act.Height,
                Layout = act.Layout,
                Entities = act.Entities.Select(x => new RSDKv3.Entity
                {
                    Type = x.Type,
